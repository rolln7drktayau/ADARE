...
\begin{enumerate}
    ...
    \item Third, we add density-aware mutation, archive guidance, and controller traces, while keeping the environmental selection mechanism fixed for fair attribution in paired experiments.
    \item Finally, we report a statistically grounded evaluation on three workflow instances with \textbf{20 paired runs} per benchmark, together with computational complexity analysis and runtime profiling.
\end{enumerate}
...
ADARE is formulated as a many-objective adaptive scheduler with three explicit control modules: Pareto-contextual operator selection, density-aware mutation and repair, and bounded archive guidance. The current artifact uses NSGA-III-style reference-vector survival as the environmental selection component because it is a strong and interpretable many-objective survival rule; the ADARE contribution is the online control policy surrounding that survival step. Figure~\ref{fig:adare_structure} summarizes this organization.
...
The artifact contains the Python implementation, JSON workflow/resource inputs, configuration files, plotting code, and paired-run driver. The main entry point is \texttt{main.py}; ADARE parameters are in \texttt{config/adare\_config.json}; benchmark inputs are under \texttt{data/benchmarks}; and controller traces are exported as per-benchmark trace and summary CSV files. This makes the adaptive-control decisions auditable rather than implicit in the final Pareto fronts.
...
To address the scale concern raised by the reviews, we added a stress-test protocol on four 1000-task Pegasus-style workflows already available in the artifact: CyberShake\_1000, Inspiral\_1000, Montage\_1000, and Sipht\_1000. Because these runs are substantially heavier on a local workstation, this protocol uses 2 paired runs, population size 80, and 15 generations, with both ADARE and NSGA-III allowed to use 10 parallel evaluator workers. The small repetition count means these results should be read as scalability evidence and not as a final significance study.

\begin{table}[H]
\caption{1000-Task Stress Test (2 paired runs, population 80, 15 generations, 10 evaluator workers)}
\label{tab:stress1000}
\centering
\scriptsize
\begin{tabularx}{\textwidth}{@{}lccccc>{\raggedright\arraybackslash}X@{}}
\toprule
Benchmark & Quality wins & Core wins & Core gain (\%) & Obj. gain (\%) & Time gain (\%) & Main caveat \\
\midrule
CyberShake\_1000 & 6/6 & 5/5 & 46.48 & 1.17 & 5.91 & Cost best slightly worse \\
Inspiral\_1000 & 6/6 & 5/5 & 16.76 & 3.18 & 1.08 & Needs more runs \\
Montage\_1000 & 6/6 & 5/5 & 23.69 & 0.89 & 1.02 & Energy best slightly worse \\
Sipht\_1000 & 4/6 & 3/5 & 8.37 & 0.24 & 0.04 & HV and epsilon weaker \\
\bottomrule
\end{tabularx}
\end{table}

Table~\ref{tab:stress1000} is important for interpretation. ADARE remains favorable on 22/24 tracked quality comparisons and 18/20 core-quality comparisons at 1000 tasks, with an average core-quality gain of 23.82\%. However, Sipht\_1000 exposes the current limit: the adaptive layer improves IGD, spacing, and coverage, but not HV or epsilon under this short budget. This confirms that the method scales beyond 24--30 tasks while also showing where longer budgets and additional baselines are still needed.
...
The extended workflow set has three scales: (i) smaller local benchmarks, (ii) Pegasus 1000-task workflows already present in the artifact, and (iii) five WfCommons-generated 3000-task workflows (Montage, Epigenomics, Seismology, SoyKB, and SRA-search). Table~\ref{tab:extended_baselines} reports ADARE core-metric wins and mean core-quality gain against each baseline.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{extended_core_gain_by_scale.png}
\caption{Mean ADARE core-quality gain across scales and extended baselines.}
\label{fig:extended_gain}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{extended_core_win_rate_by_scale.png}
\caption{ADARE core-metric win rate across scales and extended baselines.}
\label{fig:extended_wins}
\end{figure}

The extended results are more conservative and more useful than the original NSGA-III-only story. ADARE remains favorable against NSGA-III at all three scales, stays clearly ahead of NSGA-II and MOEA/D, and also remains positive against the two added adaptive families. OVEA-style is beaten strongly at 1000 and 3000 tasks, suggesting that reference-vector Q-learning operator adaptation alone is not enough for heterogeneous workflow schedules. QMOEA/D-AWA-style is more competitive but still trails ADARE by +35.96\% mean core gain on 1000-task workflows and +23.53\% on the 3000-task suite. QL-NSGA-III remains the strongest cautionary baseline: ADARE is positive on small and 3000-task workflows, while the 1000-task suite is nearly tied and slightly negative in mean core gain. This shows that adaptive operator selection is a real competitive family; ADARE's contribution is the Pareto-contextual reward, diversity-aware mutation, archive guidance, and workflow-specific control design rather than the mere presence of learning.
...
\begin{minipage}[t]{0.485\textwidth}
\centering
\includegraphics[width=\linewidth]{pareto_test_cybershake100.png}\\[-0.5ex]
\scriptsize (d) CyberShake\_100
\end{minipage}
\caption{Representative Pareto projections (ADARE vs NSGA-III) across three main workflow families, plus a larger available case (CyberShake\_100) from an exploratory paired run set.}
\label{fig:pareto_all}
\end{figure}

Figure~\ref{fig:pareto_all} is consistent with the quantitative results: ADARE extends favorable regions on Montage, broadens coverage on CyberShake, and improves spread on Epigenomics while preserving strong objective extremes. The larger CyberShake\_100 projection remains coherent with that trend and gives an additional visual stress case beyond the three main benchmarks.
...
We use population size 100 and 70 generations to keep comparisons computationally affordable on a local workstation while preserving enough diversity for four-objective optimization. This is a deliberate compromise between statistical replication and per-run search depth, but it is also a limitation. Many-objective populations can become largely non-dominated early in the run, reducing selection pressure \cite{purshouse2007many}; a stronger revised evaluation should therefore include at least one long-budget stress run (e.g., 500 generations) to verify that ADARE's advantage is not only an early exploration effect.
...
The largest remaining limitation is external validity and repetition count at scale. The added 1000-task and WfCommons 3000-task stress tests strengthen the scalability evidence, and the extended baseline suite now includes NSGA-II, MOEA/D, QL-NSGA-III, OVEA-style, and QMOEA/D-AWA-style comparisons. However, these large runs use short budgets and small repetition counts. They therefore do not replace a full large-scale statistical study with 20 paired runs, longer budgets, and exact public implementations of OVEA, QMOEA/D-AWA, or MRL-MOEA when such artifacts are available. We therefore treat the current evidence as strong for controlled workflow scheduling and promising for 1000--3000 task scale, not as a universal scalability claim.
\section{Conclusion}
% Discussion refinement
