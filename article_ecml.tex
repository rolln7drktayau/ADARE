...
% Author reasoning clarification
In practice, however, workflow scheduling studies often keep crossover/mutation schedules fixed because dynamic operator policies are harder to instrument and compare reproducibly. That engineering convenience becomes a methodological limitation in our setting. ADARE keeps the many-objective selection backbone intact (NSGA-III in the current artifact) and changes the control policy around operator choice and mutation behavior as the search state evolves.
...
% Experimental intuition added
To verify this empirically, we instrumented UCB selection/update and archive routines on 15 representative runs (5 seeds per benchmark). The measured upper-bound control ratio is \textbf{5.17\% on average} (maximum \textbf{5.34\%}) of total ADARE runtime. The instrumentation was intentionally conservative at Python level, so the ratio should be read as an upper bound rather than an exact decomposition of every micro-operation. It nevertheless supports the intended near-5\% overhead target while preserving measurable quality gains.
...
% Author reasoning clarification
We report \textbf{20 paired runs per benchmark}. The choice to execute 4 batches of 5 paired runs was an execution-stability compromise on the local workstation, not a change in statistical protocol; seed continuity was preserved when merging the CSV outputs into one 20-run dataset per benchmark.
...
ADARE improves HV on all three workflows and improves makespan/latency extremes in every case. IGD improves on CyberShake but is slightly worse on Montage and Epigenomics; this is consistent with the trade-off we observed between reinforcing extremes/spread and optimizing a distance-based metric against the reference front.
...
Table~\ref{tab:stats20} confirms that the quality gains are not only visual. HV improvements are significant on all three workflows. Epigenomics remains the most difficult case in our setting: ADARE still improves HV and spacing strongly, but runtime gains are weaker and not significant at the 0.05 level, so runtime claims on that benchmark should be interpreted cautiously.
...
These patterns support the adaptive-control interpretation: ADARE is not uniformly superior on every metric, but it improves overall search behavior in a way that is consistent with benchmark structure. This is also why we avoid benchmark-independent claims about any single metric (especially runtime and IGD) from a smaller number of repetitions.
...
Under a fairness-controlled protocol (same initialization, same budget, same evaluator) and a stronger statistical setup (20 paired runs per benchmark), ADARE improves overall many-objective search quality and runtime against NSGA-III on three heterogeneous workflow instances. The gains are strongest in HV, coverage, spacing, and objective extremes, with CyberShake showing the clearest combined benefit. At the same time, the weaker runtime significance on Epigenomics is a reminder that the effect is benchmark-dependent and that not every gain transfers with the same strength across workflow structures. Importantly, the measured control overhead remains near 5\% on average, preserving practical usability on a local workstation.
