...
With a moderate objective count, many studies still start from NSGA-II \cite{deb2002nsga2}. In our comparisons, NSGA-III keeps the same lineage and uses reference points to retain selection pressure in many-objective settings \cite{deb2014nsga3}. MOEA/D \cite{zhang2007moead} is different. It decomposes the problem into coordinated scalar subproblems. We keep this trio because it exposes distinct search biases during runs: dominance ranking, reference-point niching, and decomposition.
...
In our runs, we repeatedly observed this tension in edge/fog/cloud orchestration. Faster assignments reduce completion time. Sometimes sharply. Depending on tier placement, cost or energy can then increase. So we target a diverse, high-quality Pareto set rather than a single operating point.
