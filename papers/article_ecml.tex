...
The largest remaining limitation is external validity. The added 1000-task stress test strengthens the scalability evidence, but it uses only 2 paired runs and a short 15-generation budget. It therefore does not replace a full large-scale statistical study with 20 paired runs, longer budgets, non-scientific workflows, and direct comparisons with recent learning-assisted MOEAs. We therefore treat the current evidence as strong for controlled workflow scheduling and promising for 1000-task scale, not as a universal scalability claim.
\section{Conclusion}
% Discussion refinement
