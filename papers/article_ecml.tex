...
\begin{enumerate}
    ...
    \item Third, we add density-aware mutation, archive guidance, and controller traces, while keeping NSGA-III selection fixed for fair attribution.
...
We also added a broader comparison requested by the reviews. The extended runner evaluates ADARE against four baselines under the same evaluator, initial population, budget, and 10-worker evaluation setting: NSGA-III, NSGA-II, MOEA/D, and QL-NSGA-III. QL-NSGA-III is a compact Q-learning-assisted NSGA-III baseline inspired by adaptive operator selection; it is not an exact reproduction of OVEA or QLMOEA/D-AWA.
...
The extended results are more conservative than the original NSGA-III-only story. ADARE remains stronger than NSGA-II and MOEA/D and remains favorable against NSGA-III at all three scales. QL-NSGA-III is the strongest additional baseline: ADARE remains positive on small and 3000-task workflows, while the 1000-task suite is nearly tied and slightly negative in mean core gain. This shows that adaptive operator selection is a real competitive family; ADARE's contribution is the Pareto-contextual reward and workflow-specific control design rather than the mere presence of learning.
...
For the ECML PKDD framing, the main takeaway is that workflow scheduling in heterogeneous distributed systems benefits from treating evolutionary operator choice as a \emph{contextual online learning control problem}. Future work will extend this evaluation to larger workflow instances, add the 500-generation convergence stress test, validate the controller reward against sampled HV contribution, finalize direct NSGA-II and MOEA/D implementations, and add direct comparisons with recent learning-based state-of-the-art methods such as OVEA and QLMOEA/D-AWA under the same evaluator.
